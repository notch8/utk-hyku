...
class UriToStringConverterService
  ...
  class << self
    ...
    private
      ...
      def fetch_object(graph, subject, predicate)
        deletion = graph.predicates.find { |pred| pred.end_with?('deletionNote') }

        if deletion.present?
          object = graph.query([subject, deletion, nil]).objects.first
          return "#{subject} (Failed to load URI) - #{object}"
        end
        ...
        fetch_object(graph, new_subject, predicate)
      end
